...
- **신규 컴포넌트**: `tc-error-message`, `tc-input-with-chip`, `tc-authenticator-card`
...
### 3. tc-authenticator-card

인증기 정보를 표시하는 카드 컴포넌트입니다. TOTP, 패스키, USB 등 다양한 인증 방식을 지원합니다.

**Properties:**

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `type` | `'totp' \| 'passkey' \| 'sms' \| 'email' \| 'usb'` | `'totp'` | 인증 방식 |
| `name` | `string` | `''` | 인증기 이름 |
| `lastUsed` | `string` | `''` | 마지막 사용 시간 |
| `isDefault` | `boolean` | `false` | 기본 인증기 여부 |
...
3. **[Storybook](https://ui.transcodes.io)** - 컴포넌트 데모 확인
...
<CardGroup cols={2}>
  ...
  <Card title="Storybook" icon="palette" href="https://ui.transcodes.io">
    대화형 컴포넌트 데모
  </Card>
</CardGroup>
...
- **Stories 폴더 구조**: primitives, widgets, screens, controllers로 분리
