...
interface Authenticator {
  id: number;
  name: string;
  type: 'passkey' | 'totp' | 'usb';
  lastUsed: string;
  enabled: boolean;
}

@customElement('authenticator-list')
export class AuthenticatorList extends LitElement {
  @state()
        ...
        ${this.authenticators.map(
          (auth) => html`
            <tc-authenticator-card
              ...
              ?enabled=${auth.enabled}
              ?clickable=${true}
              @tc-delete=${() => this.handleDelete(auth.id, auth.name, auth.type)}
              @tc-click=${() => this.handleCardClick(auth)}
            ></tc-authenticator-card>
          `
        )}
      </div>
  ...
  private async handleDelete(id: number, name: string, type: string) {
    if (confirm(`"${name}"을(를) 삭제하시겠습니까?`)) {
      try {
        ...
        this.authenticators = this.authenticators.filter((a) => a.id !== id);
        console.log('인증 방식이 삭제되었습니다');
      } catch (error) {
        console.error('삭제 오류:', error);
        alert('삭제에 실패했습니다');
  ...
  private handleCardClick(authenticator: Authenticator) {
    console.log('카드 클릭:', authenticator);
  }
}
```
...
<Card title="Storybook에서 보기" href="https://transcodes-ui-ui-components.vercel.app/?path=/story/widgets-tc-authenticator-card" icon="external-link">
