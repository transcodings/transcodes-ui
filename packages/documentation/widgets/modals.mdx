...
import type { TcNotificationModal } from '@transcodes/ui-components/widgets';

@customElement('notification-demo')
export class NotificationDemo extends LitElement {
  @query('tc-notification-modal')
  private modal!: TcNotificationModal;
  ...
  render() {
    return html`
      <tc-notification-modal
        ...
        allow-label="네"
        deny-label="나중에"
        @tc-allow=${this.handleAllow}
        @tc-deny=${this.handleDeny}
...
import '@transcodes/design-tokens';
import '@transcodes/ui-components/widgets';
import type { TcOfflineModal } from '@transcodes/ui-components/widgets';

@customElement('offline-demo')
export class OfflineDemo extends LitElement {
  @query('tc-offline-modal')
  private modal!: TcOfflineModal;
  ...
  render() {
    return html`
      <tc-offline-modal
        ...
        @tc-online=${this.handleOnline}
        @tc-offline=${this.handleOffline}
      ></tc-offline-modal>
    `;
  }
...
import type { TcIframeModal } from '@transcodes/ui-components/widgets';

@customElement('iframe-modal-demo')
export class IframeModalDemo extends LitElement {
  @query('tc-iframe-modal')
  private modal!: TcIframeModal;

  render() {
    return html`
      <div>
        <tc-button @tc-click=${this.openModal}>양식 열기</tc-button>

        <tc-iframe-modal
          src="https://example.com/form"
          title="외부 양식"
          @tc-load=${this.handleLoad}
          @tc-message=${this.handleMessage}
          @tc-close=${this.handleClose}
        ></tc-iframe-modal>
      </div>
    `;
  ...
  private handleLoad() {
    console.log('iframe 콘텐츠 로드됨');
    this.modal?.sendMessage('init', { userId: '123' });
  }
